...
class Daily_News extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$term = get_field('cat', $widget_id) ?: array();
...
?>
    <?php if ($queryPosts->have_posts()) : ?>
        <div class="card-body">
            ...
            <?php if (!is_wp_error(get_term($term[0] ?? 0))): ?>
                <div class="mt-3"><a href="<?php echo get_term_link($term[0]) ?>" target="_blank" class="btn btn-outline-primary btn-block"><?php _e('More', 'pandapro') ?></a></div>
            <?php endif; ?>
        </div>
    <?php
        else:
